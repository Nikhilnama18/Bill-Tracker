...
class organizationService {
  ...
  async updateOrganisation(org: IUpdateOrganization): Promise<IOrganization[]> {
    ...
    // Fetch org for user.
    const existingOrg: IOrganization[] = await orgRepo.getOrganizationByIdAndUserId(org.u_id, org.o_id);
    if (existingOrg.length == 0)
    ...
    if (existingOrg[0].o_name != org.o_name) {
      ...
      const foundOrg: IOrganization[] = await orgRepo.getOrganizationByNameAndUserId(org.u_id, org.o_name);
